...
`ok=true` 或进程退出码为零不能替代 `status/verified`。`status` 是当前 Doc 业务契约中的状态，不应与框架外层 `outcome` 混为一谈。写回执应保留 operation、target、steps、warnings、failures、compensation 和 verification 摘要。

优先级从高到低为：稳定返回 ID/任务 ID → 写后读回匹配 → 明确服务端终态 → 仅 transport/退出码。只有前三类中与该操作匹配的证据足够时才报告成功；Runtime 已给出充分回读证据时，不为“再确认一次”重复请求。
